...
export async function SiteHeader() {
  ...
  return (
    <header className="muhub-site-header sticky top-0 z-50">
      <div className="mx-auto flex max-w-6xl items-center gap-3 px-3 py-2 sm:gap-4 sm:px-5 sm:py-2.5">
        <Link
          href="/"
        ...
        <nav
          ...
          aria-label="主导航"
        >
          <Link href="/projects" className="muhub-nav-link">
            项目广场
          </Link>
          <CreateProjectMenu authenticated={Boolean(user)} />
          {user ? (
            <>
              <Link href="/dashboard" className="muhub-nav-link">
                我的项目
              </Link>
              <Link
                href="/dashboard"
                className="relative inline-flex items-center justify-center px-1.5 py-1.5 sm:px-2 muhub-nav-link"
                aria-label={
                  unreadNotifyCount > 0

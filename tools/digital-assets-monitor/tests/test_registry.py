 ...
 def test_candidates_and_operational_channels_are_separate(self):
  with tempfile.TemporaryDirectory() as folder:
   ...
   try:
    ...
    self.assertEqual(result["channels"]["collection_enabled"],7)
    ...
    self.assertEqual(public_resources["identity_verified"],3)
    self.assertEqual(result["channels"]["endpoint_verified"],0)
    conn=db.connect();configured=conn.execute("SELECT evidence_url FROM institution_channels WHERE id='configured-public-xinjiang-property'").fetchone();conn.close()
    self.assertIsNotNone(configured)
    self.assertTrue(configured["evidence_url"].startswith("https://"))

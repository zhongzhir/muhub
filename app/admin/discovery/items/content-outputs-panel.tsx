    ...
    <section className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
      <div className="flex items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">内容结果</h2>
          <p className="text-xs text-zinc-500 dark:text-zinc-400">公众号: {wechatDraft ? 1 : 0} · X: {xDrafts.length}</p>
        </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <div className="rounded-md border border-zinc-200 p-3 dark:border-zinc-800">
          <div className="mb-2 flex items-center justify-between gap-2">
            <h3 className="text-xs font-semibold text-zinc-800 dark:text-zinc-100">公众号草稿</h3>
            {wechatDraft ? <ContentCopyButton text={wechatDraft.content} /> : null}
          </div>
          {wechatDraft ? (
            <p className="mb-2 text-[11px] text-zinc-500 dark:text-zinc-400">
              最近生成：{formatUpdatedAt(wechatDraft.updatedAt)}
            </p>
          ) : null}
          {wechatDraft ? (
        ...
        <div className="rounded-md border border-zinc-200 p-3 dark:border-zinc-800">
          <h3 className="mb-2 text-xs font-semibold text-zinc-800 dark:text-zinc-100">X 草稿</h3>
          {latestXGeneratedAt ? (
            <p className="mb-2 text-[11px] text-zinc-500 dark:text-zinc-400">
              最近生成：{formatUpdatedAt(latestXGeneratedAt)}
            </p>
          ) : null}
          {xDrafts.length === 0 ? (
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
      ...
      <div className="mt-3 space-y-1 text-[11px] text-zinc-500 dark:text-zinc-400">
        <p>来源目录：muhub-ops-engine/outputs/wechat</p>
        <p>来源目录：muhub-ops-engine/outputs/x</p>
      </div>
    </section>
  );
}

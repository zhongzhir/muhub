  ...
  async function runBulk(intent: "publish" | "hide" | "archive") {
    ...
    try {
      ...
      const json = (await res.json()) as {
        ...
        skippedCount?: number;
        warningCount?: number;
        autoGeneratedAiInsightCount?: number;
        aiInsightGenerationFailedCount?: number;
        counts?: {
          processed: number;
          published: number;
          blocked: number;
          skipped: number;
          warnings?: number;
          autoGeneratedAiInsight?: number;
          aiInsightGenerationFailed?: number;
        };
        message?: string;
      ...
      if (intent === "publish") {
        ...
        const parts = [
          ...
          typeof counts?.autoGeneratedAiInsight === "number"
            ? `自动补生成认知卡 ${counts.autoGeneratedAiInsight}`
            : typeof json.autoGeneratedAiInsightCount === "number"
              ? `自动补生成认知卡 ${json.autoGeneratedAiInsightCount}`
              : null,
          typeof counts?.aiInsightGenerationFailed === "number"
            ? `认知卡生成失败 ${counts.aiInsightGenerationFailed}`
            : typeof json.aiInsightGenerationFailedCount === "number"
              ? `认知卡生成失败 ${json.aiInsightGenerationFailedCount}`
              : null,
        ].filter(Boolean);
